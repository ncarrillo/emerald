...
use crate::{
    context::Context,
    hw::{extensions::BitManipulation, sh4::bus::PhysicalAddress},
    scheduler::ScheduledEvent,
};

use super::HollyEventData;
    ...
    pub fn read_32(&self, addr: PhysicalAddress) -> u32 {
        let ret = match addr.0 {
            ...
            0x005f688c => {
                ...
                self.registers
                    .ffst_cnt
                    .set(self.registers.ffst_cnt.get() + 1);
            ...
            0x005F74B0 => self.registers.g1_sym,
            _ => {
                println!("sb: unimplemented read (32-bit) @ 0x{:08x}", addr.0);
                0
            }
        };
        ...
        match addr.0 {
            ...
            0x005f6900 => {
                ...
                context.scheduler.schedule(ScheduledEvent::HollyEvent {
                    deadline: 0,
                    event_data: HollyEventData::RecalculateInterrupts,
                });
            }
            0x005f6904 => {
            ...
            0x005f6808 => {
                ...
                if self.registers.c2dst == 0x1 {
                    context
                        ...
                        .schedule(crate::scheduler::ScheduledEvent::HollyEvent {
                            deadline: 0,
                            event_data: super::HollyEventData::Ch2DMA,
                        })
                }
            }
            ...
            0x005f6820 => {
                self.registers.sdst = value & 1;
                if self.registers.sdst > 0 {
                    panic!("sort dma enabled");
                }
            }
            ...
            0x005f6c18 => {
                ...
                if self.registers.mdst == 1 {
                    context
                        ...
                        .schedule(crate::scheduler::ScheduledEvent::HollyEvent {
                            deadline: 0,
                            event_data: super::HollyEventData::MapleDMA,
                        });
                }
            }
            0x005f6c80 => {} // msys
            ...
            0x005f7c00 => self.registers.pdstap = value,
            0x005f7c04 => self.registers.pdstar = value,
